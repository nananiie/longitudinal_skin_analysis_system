...
export async function initializeDatabaseExample() {
    ...
    const user = db.createUser('desktop-device-001', // Unique hardware identifier
    'desktop');
 ...
 */
export async function analyzeAndStoreExample(db, userId, imagePath) {
    try {
        // Step 1: Create analysis session for this body region
        const session = db.createSession(userId, 'forehead', 'natural', 'Morning lighting, post-shower');
        ...
        const analysis = db.createAnalysis(image.image_id, session.session_id, userId, features.spotCount, features.textureScore, features.averagePigmentation, undefined, 0.95, // confidence score
        'v1.0');
        ...
        if (!baseline) {
            ...
            baseline = db.setBaseline(userId, 'forehead', analysis.analysis_id, session.session_id, features.spotCount, features.textureScore, features.averagePigmentation);
        ...
        const recommendation = generateRecommendation(features, {
        ...
        const rec = db.createRecommendation(userId, analysis.analysis_id, session.session_id, recommendation.status, recommendation.advice, comparison?.regression_detected ? 0.8 : 0.3);
 ...
 */
export function getTrendAnalysisExample(db, userId) {
    // Get 30-day trend
 ...
 */
export function generateHistoricalReportExample(db, userId) {
 ...
 */
export async function batchAnalysisExample(db, userId, imagePaths) {
 ...
 */
export function exportForAndroidExample(db, userId) {
 ...
 */
export function maintenanceExample(db) {
...
//# sourceMappingURL=database.integration.example.js.map