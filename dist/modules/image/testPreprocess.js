...
import fs from "fs";
async function runCCL(buffer, width, height, thresholdFn) {
...
async function generatePanels(samplePath, label, outputDir) {
    // Panel 1 — original
    ...
    await sharp(adaptiveBinary, { raw: { width: imgData.width, height: imgData.height, channels: 1 } })
        ...
        .toFile(adaptiveOut);
    // OTSU THRESHOLD
    const _otsuFormula = (w0, w1, mu0, mu1) => w0 * w1 * (mu0 - mu1) ** 2;
    const _adaptiveFormula = (blockMean, C = 40) => blockMean - C;
...
async function runTest() {
    ...
    await generatePanels(path.resolve("uploads/test/allie_forehead_wof_sample.jpg"), "lighter_tone", outputDir);
    await generatePanels(path.resolve("uploads/test/darkerTone.jpg"), "darker_tone", outputDir);
}
runTest();
