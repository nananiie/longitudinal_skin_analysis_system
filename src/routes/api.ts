...
// Extend Express Request to include database
declare global {
  namespace Express {
    interface Request {
      db?: AnalysisDatabase;
...
router.post('/analyze/upload', upload.single('image'), async (req: Request, res: Response) => {
  try {
    const { userId, bodyArea = 'forehead', lightingCondition = 'natural', notes,
            sunExposure, sunscreenUse, outdoorFrequency, lastSunburn } = req.body;
    const db = req.db!;
...
router.post('/analyze', async (req: Request, res: Response) => {
  try {
    const { userId, imagePath, bodyArea = 'forehead', lightingCondition = 'natural', notes,
            sunExposure, sunscreenUse, outdoorFrequency, lastSunburn } = req.body;
    const db = req.db!;
 ...
 */
router.get('/sessions/:userId/:bodyArea', (req: Request, res: Response) => {
  try {
    const { userId, bodyArea } = req.params;
    const db = req.db!;

    const sessions = db.getUserSessions(userId)
      .filter((s: any) => s.body_area === bodyArea);
 ...
 */
router.get('/comparison/:userId/:bodyArea', (req: Request, res: Response) => {
  try {
    const { userId, bodyArea } = req.params;
    const db = req.db!;
 ...
 */
router.get('/trends/:userId/:bodyArea', (req: Request, res: Response) => {
  try {
    const { userId, bodyArea } = req.params;
    const { days = 90 } = req.query;
    const db = req.db!;

    const trends = db.getTrendData(userId, parseInt(days as string));
 ...
 */
router.put('/recommendations/:recommendationId/viewed', (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/baselines/:userId', (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    const db = req.db!;
    ...
    res.json({
      ...
      baselines: baselines.map(({ area, baseline }: any) => ({
        bodyArea: area,
